...
QUnit.test("simple title", async (assert) => {
    assert.expect(1);
    env.services.title.setParts({ zopenerp: "Leansoft" });
    assert.strictEqual(env.services.title.current, "Leansoft");
});

QUnit.test("add title part", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "Leansoft", chat: null });
    assert.strictEqual(env.services.title.current, "Leansoft");
    env.services.title.setParts({ action: "Import" });
    assert.strictEqual(env.services.title.current, "Leansoft - Import");
});
...
QUnit.test("all at once", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "Leansoft", action: "Import" });
    assert.strictEqual(env.services.title.current, "Leansoft - Import");
...
QUnit.test("get title parts", async (assert) => {
    ...
    assert.deepEqual(parts, { zopenerp: "Leansoft", action: "Import" });
    parts.action = "Export";
    assert.strictEqual(env.services.title.current, "Leansoft - Import"); // parts is a copy!
});
