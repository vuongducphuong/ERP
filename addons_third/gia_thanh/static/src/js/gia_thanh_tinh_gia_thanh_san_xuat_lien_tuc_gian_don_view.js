odoo.define('gia_thanh.tinh_gia_thanh_san_xuat_lien_tuc_gian_don_form_view', function (require) {
    "use strict";
    ...
    var TinhGiaThanhSanXuatLienTucGianDonFormController = FormController.extend({
        init: function () {
        ...
        onViewLoaded: function(e, defer) {
            var self = this;
            var def =  defer;
            self.updateUI({'KY_TINH_GIA_THANH':self.params.tham_so[0],
                            'LOAI_TINH_GIA_THANH' : self.params.tham_so[1],
                            'KY_TINH_GIA_THANH_ONCHANGE':self.params.tham_so[2],
                            'TU_NGAY':self.params.tham_so[3],
                            'DEN_NGAY':self.params.tham_so[4],
                            });
            this.rpc_action({
        ...
        _onButtonClicked: function(event) {
            ...
            switch (event.data.attrs.id)
            {
                ...
                case "btn_tiep_theo":
                    ...
                    // var tong_so_tien_da_phan_bo = localData.TONG_SO_DA_PHAN_BO;
                    // var tong_so_tien_chi_phi = localData.TONG_SO_CHI_PHI;
                    // if (tong_so_tien_da_phan_bo != tong_so_tien_chi_phi){
                    //     error = 'Tổng số tiền phân bổ của TK <6271> cho các đối tượng THCP phải bằng số phân bổ lần này của TK đó. Vui lòng kiểm tra lại.';
                    //     if (error.length) {
        ...
        // onRowChanged: function(field, columnName, newValue, recordValue, record) {
        //     var localData = this.getLocalData();
        //     var so_chua_phan_bo = localData.GIA_THANH_XAC_DINH_CHI_PHI_PHAN_BO_CHI_TIET_IDS[0].SO_CHUA_PHAN_BO;
        //     var so_tien = 0;
        //     if(columnName=='TY_LE_PHAN_TRAM_6271'){
        //         so_tien = (newValue * so_chua_phan_bo)/100;
    ...
    var TinhGiaThanhSanXuatLienTucGianDonFormView = FormView.extend({
        config: _.extend({}, FormView.prototype.config, {
           
            Controller: TinhGiaThanhSanXuatLienTucGianDonFormController,
        }),
    });
    
    view_registry.add('tinh_gia_thanh_san_xuat_lien_tuc_gian_don_form_view', TinhGiaThanhSanXuatLienTucGianDonFormView);
    
    return TinhGiaThanhSanXuatLienTucGianDonFormView;
});
