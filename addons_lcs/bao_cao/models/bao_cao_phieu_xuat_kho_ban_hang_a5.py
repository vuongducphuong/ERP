...
class BAO_CAO_PHIEU_XUAT_KHO_BAN_HANG_A5(models.Model):
	_name = 'bao.cao.phieu.xuat.kho.ban.hang.a5'
	_auto = False
	...
	ACCOUNT_EX_PHIEU_THU_CHI_TIET_IDS = fields.One2many('bao.cao.phieu.xuat.kho.ban.hang.a5.chi.tiet', 'PHIEU_THU_CHI_ID', string='Phiếu thu chi tiết')



class BAO_CAO_PHIEU_XUAT_KHO_BAN_HANG_A5_REPORT(models.AbstractModel):
	_name = 'report.bao_cao.template_outwardstockbysalevouchera5'
	...
	def get_report_values(self, docids, data=None):
		...
		for line in sql_result:
			...
			master_data = self.env['bao.cao.phieu.xuat.kho.ban.hang.a5'].convert_for_update(line)
			detail_data = self.env['bao.cao.phieu.xuat.kho.ban.hang.a5.chi.tiet'].convert_for_update(line)
			if records.get(key):
				records[key].update(master_data)
				records[key].ACCOUNT_EX_PHIEU_THU_CHI_TIET_IDS += self.env['bao.cao.phieu.xuat.kho.ban.hang.a5.chi.tiet'].new(detail_data)
