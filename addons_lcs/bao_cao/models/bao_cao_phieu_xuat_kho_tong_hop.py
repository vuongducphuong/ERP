...
class BAO_CAO_PHIEU_XUAT_KHO_TONG_HOP(models.Model):
    _name = 'bao.cao.phieu.xuat.kho.tong.hop'
    _auto = False
    ...
    CHI_TIET_IDS = fields.One2many('bao.cao.phieu.xuat.kho.tong.hop.chi.tiet', 'NHAP_XUAT_ID')



class BAO_CAO_PHIEU_XUAT_KHO_TONG_HOP_REPORT(models.AbstractModel):
    _name = 'report.bao_cao.template_phieuxuatkhotonghop'
    ...
    def get_report_values(self, docids, data=None):
        ...
        for line in sql_result:
            new_key = '-'.join([line.get('MA_HANG'),line.get('TEN_HANG')])
            ###############################################
            # Updated 2019-09-25 by anhtuan: Do chưa kịp bổ
            # sung SO_LUONG_YEU_CAU vào SQL nên tự thực hiện
            ###############################################
            sl_yeu_cau = 0
            ct = self.env[line.get('MODEL_CHUNG_TU')].browse(line.get('ID_CHUNG_TU'))
            for l in ct.SOURCE_ID.SALE_DOCUMENT_LINE_IDS:
                if l.MA_HANG_ID.MA == line.get('MA_HANG'):
                    sl_yeu_cau = getattr(l,'SO_LUONG_YEU_CAU', 0)
                    break
            ###############################################
            if new_key in new_sql_result:
                new_sql_result[new_key]['SO_LUONG'] += line.get('SO_LUONG')
                new_sql_result[new_key]['SO_LUONG_YEU_CAU'] += sl_yeu_cau
            else:
                new_sql_result[new_key] = line
                new_sql_result[new_key]['SO_LUONG_YEU_CAU'] = sl_yeu_cau
        # Tạo object để in
        for k in new_sql_result:
            line = new_sql_result.get(k)
            key = str(line.get('MODEL_CHUNG_TU')) + ',' + str(line.get('ID_CHUNG_TU'))
            master_data = self.env['bao.cao.phieu.xuat.kho.tong.hop'].convert_for_update(line)
            detail_data = self.env['bao.cao.phieu.xuat.kho.tong.hop.chi.tiet'].convert_for_update(line)
            if records.get(key):
                records[key].update(master_data)
                records[key].CHI_TIET_IDS += self.env['bao.cao.phieu.xuat.kho.tong.hop.chi.tiet'].new(detail_data)
