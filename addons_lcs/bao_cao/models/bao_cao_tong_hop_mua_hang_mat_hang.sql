...
-- Description:	<Mua hàng: Lấy số liệu tổng hợp mua hàng theo mặt hàng>
-- =============================================
  ...
  DECLARE  @IsWorkingWithManagementBook BIT--  Có dùng sổ quản trị hay không?
    ...
    BEGIN
        ...
        SELECT  ROW_NUMBER() OVER ( ORDER BY PL.InventoryItemCode , LII.InventoryItemName ) AS RowNum ,
        ...
        GROUP BY PL.InventoryItemID ,--MA_HANG_ID
                PL.InventoryItemCode , -- Mã hàng--MA_HANG
                LII.InventoryItemName , -- Tên hàng -- TEN_HANG
                CASE LII.InventoryItemType WHEN 2 THEN PL.UnitID ELSE PL.MainUnitID END , -- Nếu là dịch vụ thì nhóm theo ĐVT--PL."DVT_ID" ELSE PL."DVT_CHINH_ID"
                CASE LII.InventoryItemType WHEN 2 THEN U.UnitName ELSE MU.UnitName END , -- Tên ĐVT--U."DON_VI_TINH" ELSE MU."DON_VI_TINH"
                LII.InventoryItemCategoryName
