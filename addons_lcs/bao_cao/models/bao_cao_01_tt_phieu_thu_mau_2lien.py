...
class BAO_CAO_01TT_PHIEU_THU_MAU_2LIEN(models.Model):
    _name = 'bao.cao.01tt.phieu.thu.mau.2lien'
    _auto = False
    ...
    ACCOUNT_EX_PHIEU_THU_CHI_TIET_IDS = fields.One2many('bao.cao.01tt.phieu.thu.mau.2lien.chi.tiet', 'PHIEU_THU_CHI_ID', string='Phiếu thu chi tiết')

class BAO_CAO_01TT_PHIEU_THU_MAU_2LIEN_REPORT(models.AbstractModel):
    _name = 'report.bao_cao.template_v01_tt_receipts_2lien'
    ...
    def get_report_values(self, docids, data=None):
        ...
        for line in sql_result:
            ...
            master_data = self.env['bao.cao.01tt.phieu.thu.mau.2lien'].convert_for_update(line)
            detail_data = self.env['bao.cao.01tt.phieu.thu.mau.2lien.chi.tiet'].convert_for_update(line)
            if records.get(key):
                records[key].update(master_data)
                records[key].ACCOUNT_EX_PHIEU_THU_CHI_TIET_IDS += self.env['bao.cao.01tt.phieu.thu.mau.2lien.chi.tiet'].new(detail_data)
