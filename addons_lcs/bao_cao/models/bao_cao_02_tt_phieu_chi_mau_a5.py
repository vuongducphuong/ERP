...
class BAO_CAO_02TT_PHIEU_CHI_MAU_A5(models.Model):
    _name = 'bao.cao.02tt.phieu.chi.mau.a5'
    _auto = False
    ...
    ACCOUNT_EX_PHIEU_THU_CHI_TIET_IDS = fields.One2many('bao.cao.02tt.phieu.chi.mau.a5.chi.tiet', 'PHIEU_THU_CHI_ID', string='Phiếu thu chi tiết')

    
class BAO_CAO_02TT_PHIEU_CHI_MAU_A5_REPORT(models.AbstractModel):
    _name = 'report.bao_cao.template_v02_tt_a5'
...
FROM TMP_KET_QUA --WHERE  "SO_CHUNG_TU" ='PC00031' --AND "DIEN_GIAI_DETAIL" =N'Tủ lạnh Toshiba 110 lít'
  ORDER BY
        ...
        for line in sql_result:
            ...
            master_data = self.env['bao.cao.02tt.phieu.chi.mau.a5'].convert_for_update(line)
            detail_data = self.env['bao.cao.02tt.phieu.chi.mau.a5.chi.tiet'].convert_for_update(line)
            if records.get(key):
                records[key].update(master_data)
                records[key].ACCOUNT_EX_PHIEU_THU_CHI_TIET_IDS += self.env['bao.cao.02tt.phieu.chi.mau.a5.chi.tiet'].new(detail_data)
