...
class BAO_CAO_UNC_VIETCOMBANK(models.Model):
	_name = 'bao.cao.uy.nhiem.chi.vietcombank'
	_auto = False
	...
	ACCOUNT_EX_PHIEU_THU_CHI_TIET_IDS = fields.One2many('bao.cao.uy.nhiem.chi.vietcombank.chi.tiet', 'PHIEU_THU_CHI_ID', string='Phiếu thu chi tiết')


class BAO_CAO_UNC_VIETCOMBANK_REPORT(models.AbstractModel):
	_name = 'report.bao_cao.template_vietcombankunc'
	...
	def get_report_values(self, docids, data=None):
		...
		for line in sql_result:
			...
			master_data = self.env['bao.cao.uy.nhiem.chi.vietcombank'].convert_for_update(line)
			detail_data = self.env['bao.cao.uy.nhiem.chi.vietcombank.chi.tiet'].convert_for_update(line)
			if records.get(key):
				records[key].update(master_data)
				records[key].ACCOUNT_EX_PHIEU_THU_CHI_TIET_IDS += self.env['bao.cao.uy.nhiem.chi.vietcombank.chi.tiet'].new(detail_data)
