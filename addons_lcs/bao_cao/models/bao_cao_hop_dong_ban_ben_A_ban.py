...
class BAO_CAO_HOP_DONG_BAN(models.Model):
	_name = 'bao.cao.hop.dong.ban'
	_auto = False
	...
	ACCOUNT_EX_PHIEU_THU_CHI_TIET_IDS = fields.One2many('bao.cao.hop.dong.ban.chi.tiet', 'PHIEU_THU_CHI_ID', string='Phiếu thu chi tiết')
...
class BAO_CAO_HOP_DONG_BAN_REPORT(models.AbstractModel):
	_name = 'report.bao_cao.template_hopdongban4e369ad8'
	...
	def get_report_values(self, docids, data=None):
		...
		for line in sql_result_master:
			...
			master_data = self.env['bao.cao.hop.dong.ban'].convert_for_update(line)
			if records.get(key):
		...
		for line in sql_result_detail:
			...
			detail_data1 = self.env['bao.cao.hop.dong.ban.chi.tiet'].convert_for_update(line)
			if records.get(key):
				records[key].ACCOUNT_EX_PHIEU_THU_CHI_TIET_IDS += self.env['bao.cao.hop.dong.ban.chi.tiet'].new(detail_data1)
