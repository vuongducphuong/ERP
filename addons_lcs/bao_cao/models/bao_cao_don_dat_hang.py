...
class BAO_CAO_DON_DAT_HANG(models.Model):
	_name = 'bao.cao.don.dat.hang'
	_auto = False
	...
	ACCOUNT_EX_PHIEU_THU_CHI_TIET_IDS = fields.One2many('bao.cao.don.dat.hang.chi.tiet', 'PHIEU_THU_CHI_ID', string='Phiếu thu chi tiết')



class BAO_CAO_DON_DAT_HANG_REPORT(models.AbstractModel):
	_name = 'report.bao_cao.template_saordervoucher3520'
	...
	def get_report_values(self, docids, data=None):
		...
		for line in sql_result:
			...
			master_data = self.env['bao.cao.don.dat.hang'].convert_for_update(line)
			detail_data = self.env['bao.cao.don.dat.hang.chi.tiet'].convert_for_update(line)
			if records.get(key):
				records[key].update(master_data)
				records[key].ACCOUNT_EX_PHIEU_THU_CHI_TIET_IDS += self.env['bao.cao.don.dat.hang.chi.tiet'].new(detail_data)
