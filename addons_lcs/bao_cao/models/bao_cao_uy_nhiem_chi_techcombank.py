...
class BAO_CAO_UNC_TECHCOMBANK(models.Model):
	_name = 'bao.cao.uy.nhiem.chi.techcombank'
	_auto = False
	...
	ACCOUNT_EX_PHIEU_THU_CHI_TIET_IDS = fields.One2many('bao.cao.uy.nhiem.chi.techcombank.chi.tiet', 'PHIEU_THU_CHI_ID', string='Phiếu thu chi tiết')


class BAO_CAO_UNC_TECHCOMBANK_REPORT(models.AbstractModel):
	_name = 'report.bao_cao.template_techcombankunc'
	...
	def get_report_values(self, docids, data=None):
		...
		for line in sql_result:
			...
			master_data = self.env['bao.cao.uy.nhiem.chi.techcombank'].convert_for_update(line)
			detail_data = self.env['bao.cao.uy.nhiem.chi.techcombank.chi.tiet'].convert_for_update(line)
			if records.get(key):
				records[key].update(master_data)
				records[key].ACCOUNT_EX_PHIEU_THU_CHI_TIET_IDS += self.env['bao.cao.uy.nhiem.chi.techcombank.chi.tiet'].new(detail_data)
