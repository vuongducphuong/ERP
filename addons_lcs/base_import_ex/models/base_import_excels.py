...
class Import(models.TransientModel):
    ...
    _name = 'base_import_ex.import_excels'
    ...
    parent_id = fields.Many2one('base_import_ex.import_excels')
    ...
    def importFromMS(self, tag):
        method = getattr(self, '_import_' + str(tag.tag), lambda tag: "nothing")
        return method(tag)
    ...
    def parse_preview(self, options, count=10):
        ...
        """
        self.ensure_one()
        import_ids = self.env['base_import_ex.import_excels'].search([('parent_id','=',self.id)])
        result = []
    ...
    def _parse_float_from_data(self, data, index, name, options):
        ...
        for num, line in enumerate(data):
            ...
            if line[index] is False:
                raise ValueError(_("Dòng thứ %d: Cột <%s> chứa giá trị không hợp lệ (%s)" % (num+1, name, old_value)))
                    ...
                    for num, line in enumerate(data):
                        ...
                        if line[index]:
                            try:
                                ...
                                raise ValueError(_("Dòng thứ %d: Cột <%s> chứa giá trị không hợp lệ: %s") % (num + 1, name, e))
                            except Exception as e:
                                raise ValueError(_("Giá trị không hợp lệ [%s:L%d]: %s") % (name, num + 1, e))
    ...
    def do(self, fields, options, dryrun=False):
        ...
        for import_id in import_ids:
            ...
            model = model.with_context(auto_create=options.get('auto_create', False))
            try:
                import_result = model.import_from_excel(import_fields, data)
            # except psycopg2.Error as e:
            #     import_result = {'messages': [{'type':'error', 'message' : e.pgerror}]}
            #     pass
            except Exception as e:
                import_result = {'messages': [{'type':'error', 'message' : getattr(e, 'name', str(e))}]}
                pass

            if import_result.get('messages'):
                for m in import_result['messages']:
                    m.update({'file_name': 'Mau_%s.xls' % import_id.res_model.replace('.','_')})
            _logger.info('done')
        ...
        try:
            # Nếu lỗi thì ROLLBACK, ngược lại thì release luôn
            if import_result.get('messages'):
        ...
        return import_result.get('messages')
