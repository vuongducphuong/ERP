odoo.define('base_import_ex.import', function (require) {
"use strict";
...
var DataImport = Widget.extend(ControlPanelMixin, {
    ...
    renderButtons: function() {
        var self = this;
        this.$buttons = $(QWeb.render("ImportViewEX.buttons", this));
    ...
    onfile_loaded: function () {
        ...
        jsonp(this.$el, {
            url: '/base_import_ex/set_single_file'
    ...
    onpreview_error: function (event, from, to, result) {
        ...
        this.$('.oe_import_error_report').html(
                QWeb.render('ImportViewEX.preview.error', result));
    },

    // Cho trường hợp chỉ import 1 file 
    ...
    onpreview_success_1: function (event, from, to, result) {
        var self = this;
        _.each(result, function(result) {
            ...
            self.$buttons.filter('.o_import_button').add(self.$('.oe_import_file_reload'))
                    .prop('disabled', false);
            self.$el.addClass('oe_import_preview');
            self.$('input.oe_import_advanced_mode').prop('checked', result.advanced_mode);
            self.$('.oe_import_grid').append(QWeb.render('ImportViewEX.preview', result));
    ...
    onresults: function (event, from, to, message) {
        ...
        this.$('.oe_import_error_report').html(
            QWeb.render('ImportViewEX.error', {
...
core.action_registry.add('import_ex', DataImport);
...
StateMachine.create({
    ...
    events: [
        ...
        { name: 'reset', from: 'imported', to: 'none' }
    ]
});
