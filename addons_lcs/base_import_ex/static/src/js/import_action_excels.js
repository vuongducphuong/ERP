odoo.define('base_import_ex.import_excels', function (require) {
"use strict";
...
var DataImport = Widget.extend(ControlPanelMixin, {
    ...
    events: {
        ...
        'change input.oe_show_detail': function (e) {
			if ($(e.currentTarget).prop('checked')) {
			    this.$('.oe_import_with_file.oe_padding').show();
			} else {
			    this.$('.oe_import_with_file.oe_padding').hide();
			}
        },
    ...
    init: function (parent, action) {
        ...
        action.display_name = _t('Nhập khẩu dữ liệu'); // Displayed in the breadcrumbs
        this.do_not_change_match = false;
    },
    start: function () {
        var self = this;
        var href="/base_import_ex/static/csv/Nhập khẩu từ excel.zip"
        var model_name = this.res_model;
        if (model_name) {
            var new_model_name = model_name;
            for (var i in this.parent_context) {
                if (i.startsWith('default_')) {
                    new_model_name += ('_' + this.parent_context[i]);
                }
            }
            href = '/base_import_ex/static/csv/Mau_' +  new_model_name.toLowerCase().replace(/\./g, "_") + '.xls';
        }
        this.$('.sample_file_href').prop('href', href);
    ...
    create_model: function() {
        return this._rpc({
                model: 'base_import_ex.import_excels',
                method: 'create',
    ...
    renderButtons: function() {
        var self = this;
        this.$buttons = $(QWeb.render("ImportViewExcels.buttons", this));
    ...
    import_options: function () {
        ...
        var options = {
            ...
            auto_create: this.$('input.oe_import_auto_create').prop('checked'),
    ...
    onfile_loaded: function () {
        var file = this.$('.oe_import_file')[0].files;
        var name  = file[0].name;
        this.$('.oe_import_file_show').val(name);
        ...
        var files = this.$('input.oe_import_file')[0].files;
        if (!this.res_model) {
            var accept_list = ['1. Số dư tài khoản.xls','2. Số dư TK ngân hàng.xls','3. Công nợ khách hàng.xls','4. Công nợ nhà cung cấp.xls','5. Công nợ nhân viên.xls','6. Tồn kho vật tư hàng hóa.xls','7. Chi phí dở dang - Công trình.xls','8. Chi phí dở dang - Đối tượng THCP.xls','9. Chi phí dở dang - Đơn hàng.xls','10. Chi phí dở dang - Hợp đồng.xls']
            for (var i=0; i<files.length; i++) {
                ...
                // Check file name
                if (accept_list.indexOf(file.name) < 0) {
                    alert("Tên file <<" + file.name + ">> không hợp lệ. Danh sách file có thể nhập khẩu:\
                        \n1. Số dư tài khoản.xls\
                        \n2. Số dư TK ngân hàng.xls\
                        \n3. Công nợ khách hàng.xls\
                        \n4. Công nợ nhà cung cấp.xls\
                        \n5. Công nợ nhân viên.xls\
                        \n6. Tồn kho vật tư hàng hóa.xls\
                        \n7. Chi phí dở dang - Công trình.xls\
                        \n8. Chi phí dở dang - Đối tượng THCP.xls\
                        \n9. Chi phí dở dang - Đơn hàng.xls\
                        \n10. Chi phí dở dang - Hợp đồng.xls");
                    this['reset']();
                    return;
                }
            }
        ...
        jsonp(this.$el, {
            url: '/base_import_ex/set_multi_file'
    ...
    onpreviewing: function () {
        ...
        // if (this.$('input.oe_show_detail').prop('checked')) {
        //     this.$('.oe_import_with_file.oe_padding').show();
        // } else {
        //     this.$('.oe_import_with_file.oe_padding').hide();
        // }
    ...
    onpreview_error: function (event, from, to, result) {
        ...
        this.$('.oe_import_error_report').html(
                QWeb.render('ImportViewExcels.preview.error', result));
    },

    // Cho trường hợp import nhiều file cùng lúc
    onpreview_success: function (event, from, to, results) {
        var self = this;
        self.$buttons.filter('.o_import_import').addClass('btn-primary').prop('disabled', false);
        self.$('.oe_import_file_reload').prop('disabled', false);
        self.$('.oe_import_grid').empty();
        _.each(results, function(result) {
            // self.$buttons.filter('.o_import_import').removeClass('btn-primary');
            // self.$buttons.filter('.o_import_validate').addClass('btn-primary');
            self.$el.addClass('oe_import_preview');
            self.$('input.oe_import_advanced_mode').prop('checked', result.advanced_mode);
            self.$('.oe_import_grid').append(QWeb.render('ImportViewExcels.preview', result));

            if (result.headers.length === 1) {
                self.$('.oe_import_options').show();
                self.onresults(null, null, null, [{
                    type: 'warning',
                    message: _t("A single column was found in the file, this often means the file separator is incorrect")
                }]);
            }

            self.$('.oe_import_date_format').val(time.strftime_to_moment_format(result.options.date_format));
            self.$('.oe_import_datetime_format').val(time.strftime_to_moment_format(result.options.datetime_format));
            self.$('.oe_import_float_thousand_separator').val(result.options.float_thousand_separator).change();
            self.$('.oe_import_float_decimal_separator').val(result.options.float_decimal_separator).change();
            if (result.debug === false){
                self.$('.oe_import_tracking').hide();
                self.$('.oe_import_deferparentstore').hide();
            }
            // Get lastest n elements
            var num_headers = result.headers.length;
            var $fields = self.$('.oe_import_fields input').slice(num_headers * -1);
            self.render_fields_matches(result, $fields);
            var data = self.generate_fields_completion(result);
    ...
    onresults: function (event, from, to, message) {
        ...
        for (var m=0; m<message.length; m++) {
            if (message[m].type != 'warning') {
                no_error = false;
                break;
            }
        }
        this.$buttons.filter('.o_import_import').prop('disabled', !no_error);
        ...
        this.$('.oe_import_error_report').html(
            QWeb.render('ImportViewExcels.error', {
                errors: _(message).groupBy('message'),
                at: function (rows) {
                    var from = rows.from + offset;
                    var to = rows.to + offset;
                    var file_name = message[0].file_name? ' trong file ' +  message[0].file_name : '';
                    if (from === to) {
                        return _.str.sprintf(_t("at row %d"), from) + file_name;
                    }
    ...
    onreset: function() {
        this.$('input.oe_import_file_show').val(null);
    },
});
core.action_registry.add('import_excel_ex', DataImport);
...
StateMachine.create({
    ...
    events: [
        ...
        { name: 'reset', from: ['imported','file_loaded'], to: 'none' }
    ]
});
