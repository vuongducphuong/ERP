...
class ACCOUNT_CONG_NO_NHA_CUNG_CAP_EXCEL(models.Model):
	_name = 'cong.no.nha.cung.cap'
	_description = ''
	_auto = False
	...
	SO_CON_PHAI_TRA_QUY_DOI = fields.Float(string='Số còn phải trả quy đổi', help='Số còn phải trả quy đổi')
	SO_TRA_TRUOC = fields.Float(string='Số trả trước', help='Số trả trước')
	SO_TRA_TRUOC_QUY_DOI = fields.Float(string='Số trả trước quy đổi', help='Số trả trước quy đổi')
	...
	def import_from_excel(self, import_fields, data):
		...
		for data in data_field:
			...
			doi_tuong = False
			ten_doi_tuong = False
			if data.get('MA_NHA_CUNG_CAP') != '':
				dt = self.env['res.partner'].search([('MA', '=', data.get('MA_NHA_CUNG_CAP'))])
				if dt:
					doi_tuong = dt.id
					ten_doi_tuong = dt.HO_VA_TEN
					if dt.LA_NHA_CUNG_CAP == False:
						thong_bao = u'Cột Mã nhà cung cấp (*) dòng thứ ' + str(i) + u' không tìm thấy ' + str(data.get('MA_NHA_CUNG_CAP')) + ' trong danh mục/nhà cung cấp. Vui lòng kiểm tra lại'
						error.append({'type':'error', 'message' : thong_bao})
				else:
					thong_bao = u'Cột Mã nhà cung cấp (*) dòng thứ ' + str(i) + u' sai kiểu dữ liệu hoặc chưa tồn tại trong danh mục/nhà cung cấp'
					error.append({'type':'error', 'message' : thong_bao})
			else:
				thong_bao = u'Cột Mã nhà cung cấp (*) thứ ' + str(i) + u' là trường bắt buộc nhập vui long nhập để import'
				error.append({'type':'error', 'message' : thong_bao})
			# Loại tiền
			loai_tien = False
			...
			du_lieu_1_dong = {
				...
				'MA_NHA_CUNG_CAP' : data.get('MA_NHA_CUNG_CAP') if data.get('MA_NHA_CUNG_CAP') != '' else False, # Mã nhà cung cấp
		...
		if du_lieu_trong_dtb:
			for du_lieu in du_lieu_trong_dtb:
				if du_lieu.TK_ID.DOI_TUONG == True and du_lieu.TK_ID.DOI_TUONG_SELECTION == '0':
						...
						if du_lieu.ACCOUNT_EX_SO_DU_TAI_KHOAN_CHI_TIET_THEO_DOI_TUONG_IDS:
							for ct_theo_doi_tuong in du_lieu.ACCOUNT_EX_SO_DU_TAI_KHOAN_CHI_TIET_THEO_DOI_TUONG_IDS:
								du_lieu_1_dong = {
									...
									'MA_NHA_CUNG_CAP' : du_lieu.DOI_TUONG_ID.MA, # Mã nhân viên
				...
				for gia_ben_trong in dict_create[gia_tri]:
					...
					for dong_chi_tiet in dict_chi_tiet:
						...
						if dong_chi_tiet[2].get('DU_NO') != False or dong_chi_tiet[2].get('DU_NO_NGUYEN_TE') or dong_chi_tiet[2].get('DU_CO') or dong_chi_tiet[2].get('DU_CO_NGUYEN_TE'):
							...
							du_no += dong_chi_tiet[2].get('SO_THU_TRUOC')
							du_no_nguyen_te += dong_chi_tiet[2].get('SO_THU_TRUOC_NGUYEN_TE')
							du_co += dong_chi_tiet[2].get('SO_CON_PHAI_THU')
							du_co_nguyen_te += dong_chi_tiet[2].get('SO_CON_PHAI_THU_NGUYEN_TE')

						if dong_chi_tiet[2].get('CONG_TRINH_ID') != False or dong_chi_tiet[2].get('HOP_DONG_MUA_ID') != False \
								or dong_chi_tiet[2].get('DON_MUA_HANG_ID') != False or dong_chi_tiet[2].get('NHAN_VIEN_ID') != False :	
