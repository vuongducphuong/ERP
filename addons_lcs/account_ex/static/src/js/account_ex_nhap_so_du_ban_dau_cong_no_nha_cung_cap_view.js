odoo.define('account_ex.account_ex_nhap_so_du_ban_dau_cong_no_nha_cung_cap_list_view', function (require) {
    "use strict";
...
//             Lấy số tài khoản ở master form Nhập số dư tài khoản gửi về form bật ra theo link 
            ...
            if (colName == 'NHAP_SO_DU_CHI_TIET') {
                return new dialogs.FormViewDialog(self, {
                    ...
                    recordID: record.id,
                    res_id: record.id,
                    title: "Nhập chi tiết công nợ nhà cung cấp",
                    ref_views: [['account_ex.view_account_ex_nhap_chi_tiet_cong_no_ncc_form', 'form']],
                    readonly: false,
                            ...
                            if(tong_du_no_quy_doi == 0 && tong_du_co_quy_doi == 0 && tong_du_no_nguyen_te == 0 && tong_du_co_nguyen_te == 0){
                                if(record.data.ACCOUNT_EX_SO_DU_TAI_KHOAN_CHI_TIET_THEO_HOA_DON_IDS.data.length > 0){
                                    for(var j in record.data.ACCOUNT_EX_SO_DU_TAI_KHOAN_CHI_TIET_THEO_HOA_DON_IDS.data){
                                        tong_du_co_quy_doi += record.data.ACCOUNT_EX_SO_DU_TAI_KHOAN_CHI_TIET_THEO_HOA_DON_IDS.data[j].data.SO_CON_PHAI_THU;
                                        tong_du_no_quy_doi += record.data.ACCOUNT_EX_SO_DU_TAI_KHOAN_CHI_TIET_THEO_HOA_DON_IDS.data[j].data.SO_THU_TRUOC;
                                        tong_du_co_nguyen_te += record.data.ACCOUNT_EX_SO_DU_TAI_KHOAN_CHI_TIET_THEO_HOA_DON_IDS.data[j].data.SO_CON_PHAI_THU_NGUYEN_TE;
                                        tong_du_no_nguyen_te += record.data.ACCOUNT_EX_SO_DU_TAI_KHOAN_CHI_TIET_THEO_HOA_DON_IDS.data[j].data.SO_THU_TRUOC_NGUYEN_TE;
                                    }
                                }
                            }
        ...
        onViewLoaded: function(e, defer) {
            ...
            self.changeFieldValue('currency_id', pr.loai_tien_so_so_cong_no_nha_cung_cap_id.id);
            self.changeFieldValue('SO_TAI_KHOAN_ID', pr.tai_khoan_id);
            if(pr.loai_tien_so_so_cong_no_nha_cung_cap_id.display_name == "VND"){
...
//                 self.changeFieldValue('SO_TAI_KHOAN_ID', pr.tai_khoan_id);
            ...
            this.rpc_action({
                ...
                args: {
                    ...
                    'loai_tien_id' : pr.loai_tien_so_so_cong_no_nha_cung_cap_id.id,
...
//                         self.changeFieldValue('SO_TAI_KHOAN_ID',4017);
    ...
    view_registry.add('account_ex_nhap_so_du_ban_dau_cong_no_nha_cung_cap_list_view', PurchaseDocumentView);
