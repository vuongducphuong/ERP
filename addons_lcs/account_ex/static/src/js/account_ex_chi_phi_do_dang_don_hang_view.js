odoo.define('account_ex.account_ex_chi_phi_do_dang_don_hang_form_view', function (require) {
    "use strict";
    ...
    var ChiPhiDoDangDonHangController = FormController.extend({
        ...
        onViewLoaded: function(e, defer) {
            ...
            this.rpc_action({
                ...
                method: 'lay_du_lieu_don_hang',
                // args: {},
                callback: function(result) {
                    if (result) {
                        self.updateUI(result).then(function() {
                            var fieldWidget = self.getFieldWidget('ACCOUNT_EX_CHI_PHI_DO_DANG_IDS');
                            var nhap_chi_tiet = self.getFieldValue('NHAP_CHI_TIET_LOAI_CPDD_DON_HANG');
                            if(nhap_chi_tiet == true){
                                fieldWidget.do_visible_columns(['CHI_PHI_NVL_GIAN_TIEP','CHI_PHI_NHAN_CONG_GIAN_TIEP','KHAU_HAO','CHI_PHI_MUA_NGOAI','CHI_PHI_KHAC','CHI_PHI_KHAU_HAO'], true);
        ...
        onFieldChanged: function(field){
            var self = this;
            if("NHAP_CHI_TIET_LOAI_CPDD_DON_HANG" == field){
					...
					.then(function(result) {
                        var fieldWidget = self.getFieldWidget('ACCOUNT_EX_CHI_PHI_DO_DANG_IDS');
						if(result == true){
                            if(self.getFieldValue('NHAP_CHI_TIET_LOAI_CPDD_DON_HANG') == true){
                                fieldWidget.do_visible_columns(['CHI_PHI_NVL_GIAN_TIEP','CHI_PHI_NHAN_CONG_GIAN_TIEP','KHAU_HAO','CHI_PHI_MUA_NGOAI','CHI_PHI_KHAC','CHI_PHI_KHAU_HAO'], true);
							    fieldWidget.do_visible_columns(['CHI_PHI_CHUNG'], false);
                            }
                            else{
    ...
    var ChiPhiDoDangDonHangView = FormView.extend({
        config: _.extend({}, FormView.prototype.config, {
            Controller: ChiPhiDoDangDonHangController,
        }),
    });
    
    view_registry.add('account_ex_chi_phi_do_dang_don_hang_form_view', ChiPhiDoDangDonHangView);
