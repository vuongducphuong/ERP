...
-- Description:	Store lấy dữ liệu cho chi tiết đối trừ bán hàng (chứng từ thanh toán)
-- nmtruong:11/5/2016 sửa bug 101936 thay refdate = posteddate để ghi xuống bảng đối trừ đúng ngày hạch toán
--NVTOAN modify 14/04/2017 thực hiện CR 34929 - Lấy bổ sung thông tin nhân viên
-- =============================================
...
SET			@ToDate='2019-05-25 00:00:00'
    ...
    BEGIN
        ...
        SELECT  AL.RefID ,
                ...
                $0 AS CrossEntryAmountOC ,
                $0 AS CrossEntryAmount ,
                @AccountObjectID AS AccountObjectID ,
                AL.EmployeeID , --NVTOAN modify 14/04/2017 thực hiện CR 34929
                E.AccountObjectCode AS EmployeeCode , --nvtoan Modify 22/04/2017: Sửa lỗi 96455                
                E.AccountObjectName AS EmployeeName
        FROM    ( 
				SELECT    OPNTemp.RefID ,
                            OPNTemp.RefType ,
                            OPNTemp.RefTypeName ,
                            OPNTemp.RefDate ,
                            OPNTemp.PostedDate AS PostedDate -- nmtruong: 11/5/2016 sửa bug 101936 thay refdate = posteddate để ghi xuống bảng đối trừ đúng ngày hạch toán
                            ,
                            OPNTemp.RefNo ,
                            OPNTemp.InvNo ,
                            OPNTemp.[Description] ,
                            ...
                            OPNTemp.EmployeeID  --NVTOAN modify 14/04/2017 thực hiện CR 34929
                  FROM      ( SELECT    OPN.RefID ,
                                        ...
                                        SUM(CASE WHEN OPI.RefID IS NOT NULL
                                                 THEN ISNULL(OPI.PayAmountOC,
                                                             0)
                                                 ELSE ISNULL(OPN.CreditAmountOC,
                                                             0)
                                                      - ISNULL(OPN.DebitAmountOC,
                                                              0)
                                            END) AS AmountOC ,
                                        SUM(CASE WHEN OPI.RefID IS NOT NULL
                                                 THEN ISNULL(OPI.PayAmount, 0)
                                                 ELSE ISNULL(OPN.CreditAmount,
                                                             0)
                                                      - ISNULL(OPN.DebitAmount,
                                                              0)
                                            END) AS Amount ,
                                        SUM(CASE WHEN OPI.RefID IS NOT NULL
                                                 THEN ISNULL(OPI.PayAmountOC,
                                        ...
                                        OPN.AccountNumber ,
                                        1 AS DetailByInvNo ,
                                        OPID2.EmployeeID--NVTOAN modify 14/04/2017 thực hiện CR 34929
                              ...
                              SELECT    AL.RefID ,
                                        ...
                                        SUM(ISNULL(AL.CreditAmountOC, 0)
                                            - ISNULL(AL.DebitAmountOC, 0)) AS ReceiptAmountOC ,
                                        SUM(ISNULL(AL.CreditAmount, 0)
                                            - ISNULL(AL.DebitAmount, 0)) AS ReceiptAmount ,
                                        @AccountObjectID AS AccountObjectID ,
                                        AL.AccountNumber ,
                                        0 AS DetailByInvNo ,
                                        CASE WHEN GVDT.RefID IS NOT NULL
                                             ...
                                             ELSE AL.EmployeeID
                                        END AS EmployeeID--NVTOAN modify 14/04/2017 thực hiện CR 34929
                              FROM      AccountObjectLedger AS AL
				...
				--Không lấy các chứng từ Phiếu thu tiền khách hàng, Phiếu thu tiền khách hàng hàng loạt, Phiếu chi trả tiền nhà cung cấp, Phiếu thu tiền gửi khách hàng, Phiếu thu tiền gửi khách hàng hàng loạt, Trả tiền nhà cung cấp bằng Ủy nhiệm chi, Séc tiền mặt trả tiền nhà cung cấp, Séc chuyển khoản trả tiền nhà cung cấp, Chứng từ xử lý chênh lệch tỷ giá, Chứng từ bù trừ công nợ, Xử lý chênh lệch tỷ giá từ đánh giá lại tài khoản ngoại tệ, Xử lý chênh lệch tỷ giá từ tính tỷ giá xuất quỹ
                            ...
                            LEFT JOIN dbo.Func_SA_GetPaiedAmountForPayVoucher(@AccountNumber,
                                                              @CurrencyID,
                                                              @BranchID,
                ...
                LEFT JOIN ( SELECT  CASE WHEN @AccountingSystem = '15'
                            ...
                            WHERE   GV.BranchID = @BranchID
                                    ...
                                    AND ( ( @IsWorkingManagementBook = 1
                                     ...
                                     AND GV.RowNumber = 1
                LEFT JOIN dbo.AccountObject E ON AL.EmployeeID = E.AccountObjectID --NVTOAN modify 14/04/2017 thực hiện CR 34929
        GROUP BY AL.RefID ,
                AL.RefType ,
                AL.RefTypeName ,
                AL.RefDate ,
