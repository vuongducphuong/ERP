...
class KetoanBaoCaoQuyTienMat(models.TransientModel):
    _inherit = "account.report"
    _name = 'ketoan.bao.cao.quy.tien.mat'
    ...
    def view_report(self, option, title):
        r = self.env['ketoan.bao.cao.quy.tien.mat'].search([('id', '=', option[0])])
        self = r        
    ...
    def create(self, vals):
        ...
        vals['name'] = 'eee'
        res = super(KetoanBaoCaoQuyTienMat, self).create(vals)
        return res
    ...
    def write(self, vals):
        ...
        res = super(KetoanBaoCaoQuyTienMat, self).write(vals)
        return res
    ...
    def get_accounts_line(self, account_id, title):
        ...
        # Get move lines base on sql query and Calculate the total balance of move lines
        sql = ('''SELECT l.id AS lid,m.id AS move_id, l.account_id AS account_id, l.date AS ldate, l.currency_id, l.amount_currency, l.ref AS lref, l.name AS lname, COALESCE(SUM(l.debit),0) AS debit, COALESCE(SUM(l.credit),0) AS credit, COALESCE(SUM(l.balance),0) AS balance,\
                    ...
                    LEFT JOIN account_account_tag_account_move_line_rel acc ON (acc.account_move_line_id=l.id)
                    JOIN account_account a ON (l.account_id = a.id) '''
               + WHERE + new_final_filter + ''' GROUP BY l.id, m.id,  l.account_id, l.date, l.currency_id, l.amount_currency, l.ref, l.name, m.name, c.symbol, c.position, p.name ORDER BY l.date''')
    ...
    def get_dynamic_xlsx_report(self, data, response, report_data, dfr_data):
        ...
        sheet.merge_range('A5:J6', '  Accounts: ' + ', '.join(
            [lt or '' for lt in
